...
public class BackupController {
    ...
    private void listAffectedFilesFromBackupSources() {
        ...
        try (var uniquePathFileSet = listSources(fileSetRepository)) {
            ...
            this.filesFound = threadPool.submit(() -> LongStream
                            .iterate(0L, offset -> offset < totalUniquePaths, offset -> offset + PAGE_SIZE)
                            .parallel()
                            .mapToObj(offset -> fileSetRepository.findAll(uniquePathFileSet, offset, PAGE_SIZE))
