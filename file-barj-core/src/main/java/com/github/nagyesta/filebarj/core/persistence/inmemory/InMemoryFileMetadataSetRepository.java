...
public class InMemoryFileMetadataSetRepository
    ...
    public long countByType(
            @NonNull final FileMetadataSetId id,
            @NonNull final Collection<FileType> types) {
    ...
    @Override
    public SortedMap<FileType, Long> countsByType(final @NonNull FileMetadataSetId id) {
        return new TreeMap<>(getFileSetById(id).stream()
                .collect(Collectors.groupingBy(FileMetadata::getFileType, Collectors.counting())));
    }

    @Override
    public SortedMap<Change, Long> countsByStatus(final @NonNull FileMetadataSetId id) {
        return new TreeMap<>(getFileSetById(id).stream()
