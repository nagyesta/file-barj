...
public class InMemoryFileSetRepository implements FileSetRepository {
    ...
    @Override
    public void appendTo(final @NonNull FileSetId id, final @NonNull Path path) {
        appendTo(id, Collections.singletonList(path));
    }

    @Override
    public void appendTo(final @NonNull FileSetId id, final @NonNull Collection<Path> paths) {
    ...
    @Override
    public List<Path> findAll(final @NonNull FileSetId id, final long offset, final long limit) {
        final var lock = findLockFor(id);
        lock.lock();
        try {
            return fileSets.get(id.id()).stream().sorted().skip(offset).limit(limit).toList();
        } finally {
            lock.unlock();
        }
    }
