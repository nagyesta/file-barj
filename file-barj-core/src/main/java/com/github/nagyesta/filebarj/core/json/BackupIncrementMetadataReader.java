...
public class BackupIncrementMetadataReader implements AutoCloseable {
        ...
        for (nextToken = jsonParser.nextToken(); nextToken != JsonToken.END_OBJECT; nextToken = jsonParser.nextToken()) {
            ...
            switch (fieldName) {
                ...
                case JOB_CONFIGURATION:
                    processJobConfiguration(manifestBuilder);
                    break;
                case BACKUP_VERSIONS:
                    processBackupVersions(manifestBuilder);
                    break;
                case ENCRYPTION_KEYS:
                    processEncryptionKeys(nextToken, manifestBuilder);
                    break;
                case BACKUP_TYPE:
    ...
    private void processJobConfiguration(
            final BackupIncrementManifestBuilder<?, ? extends BackupIncrementManifestBuilder<?, ?>> manifestBuilder) throws IOException {
        jsonParser.nextToken();
        final var configuration = jsonParser.readValueAs(BackupJobConfiguration.class);
        manifestBuilder.configuration(configuration);
    }

    private void processBackupVersions(
            final BackupIncrementManifestBuilder<?, ? extends BackupIncrementManifestBuilder<?, ?>> manifestBuilder) throws IOException {
        jsonParser.nextToken();
    ...
    private void processEncryptionKeys(
