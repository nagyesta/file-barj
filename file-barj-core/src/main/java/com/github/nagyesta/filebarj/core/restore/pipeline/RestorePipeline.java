...
public class RestorePipeline implements Closeable {
    ...
    public void restoreDirectories(final @NonNull FileMetadataSetId directories) {
        ...
        fileMetadataSetRepository.forEachByChangeStatusesAndFileTypes(
                ...
                dataRepositories.getSingleThreadedPool(),
    ...
    public void restoreFiles(
        ...
        final var changeStatus = detectChanges(fileMetadataSetRepository, contentSources, threadPool, false, PARSE_METADATA);
        ...
        final var archivedEntriesOfLastManifest = manifest.getArchivedEntriesOfLastManifest();
        try (var restoreScope = new RestoreScope(filesFromLastIncrement, archivedEntriesOfLastManifest, changeStatus, contentSources)) {
    ...
    public void finalizePermissions(
        ...
        try (var changeStatus = detectChanges(fileMetadataSetRepository, files, threadPool, false, VERIFY_CONTENT);
             var filesWithMetadataChanges = fileMetadataSetRepository.keepChangedMetadata(files, changeStatus)) {
            ...
            fileMetadataSetRepository.forEachReverse(filesWithMetadataChanges, dataRepositories.getSingleThreadedPool(),
                    fileMetadata -> {
    ...
    public void deleteLeftOverFiles(
        ...
        try (var pathsInRestoreTarget = fileSetRepository.createFileSet();
            ...
            fileMetadataSetRepository.forEach(filesFromLastIncrement, threadPool, fileMetadata -> {
                ...
                //TODO: should we restrict this to the pathRestriction?
                fileSetRepository.appendTo(pathsInBackup, path);
            });
            final var counter = new AtomicInteger(0);
            final var leftOverFiles = fileSetRepository.subtract(pathsInRestoreTarget, pathsInBackup);
    ...
    public void evaluateRestoreSuccess(
        ...
        try (var checkOutcome = detectChanges(fileMetadataSetRepository, files, threadPool, true, VERIFY_METADATA);
             var changedFiles = fileMetadataSetRepository.keepChangedMetadata(files, checkOutcome)) {
    ...
    private @NotNull List<Map<ArchiveEntryLocator, SortedSet<FileMetadata>>> partition(
        ...
        for (final var file : input) {
            ...
            final var fileMetadataList = dataRepositories.getArchivedFileMetadataSetRepository()
